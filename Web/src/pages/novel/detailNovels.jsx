...
import useGetNovel from '~/hooks/novel/useGetNovel';
import { useContext, useState, useEffect } from 'react';
import { UserContext } from '~/context/authContext';
import useFollowNovel from '~/hooks/novel/useFollowNovel';
import Footer from '~/components/footer/Footer';
import useComment from '../../hooks/comment/useComment.js';
import NavbarAdmin from '~/components/Navbar/NavbarAdmin';
import { FaStar } from 'react-icons/fa';
import useRatingNovel from '~/hooks/novel/useRatingNovel.js';

function DetailNovel() {
    const { id } = useParams();
    const { allowAccess } = useContext(UserContext);
    const { novel } = useGetNovel(id);
    const { FollowNovel } = useFollowNovel();
    const [isFollowed, setIsFollowed] = useState(false);
    const [message, setMessage] = useState('');
    const [genresList, setGenresList] = useState([]);

    const { RatingNovel, rate, setRate } = useRatingNovel(id);
    ...
    const handleFollow = async (e) => {
        e.preventDefault();
        try {
            const { response, data } = await FollowNovel(id);
                            ...
                            <div className="row">
                                <div className="col-lg-3">
                                    <div className="anime__details__pic" data-setbg="img/anime/details-pic.jpg">
                                        <img src={novel?.poster_img} />
                                        <div className="view">
                                            <i className="fa fa-eye"> View: {novel?.viewed}</i>
                                        </div>
                                    </div>
                                ...
                                <div className="col-lg-9">
                                    <div className="anime__details__text">
                                                ...
                                                <div className="col-lg-6">
                                                    <ul>
                                                        <li>
                                                            ...
                                                            {convertToVietnamDate(novel?.releaseDate)}
                                                        </li>
                                                        <li>
                                                            <span>TotalChap:</span> {novel?.totalChap}
                                                        </li>
                                                        <li>
                                                            <span>StoryDuration:</span> {novel?.storyDuration } phút
                                                        </li>
                                                        <li>
                                                            <span>Views:</span> {novel?.viewed}
                                                        </li>
                                                        <li>
                                                            <span>Rate:</span>{`${(novel?.totalPoints/novel?.countRating).toFixed(1)} điểm / ${novel?.countRating} lượt`} 
                                                        </li>
                                                    </ul>
                                                </div>
                                            </div>
                                        ...
                                        <div className="anime__details__btn">
                                            <Link className="watch-btn" to={`/novel/watch/${novel?._id}`}>
                                                Read Now
                                            </Link>
